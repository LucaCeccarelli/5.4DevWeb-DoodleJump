...
class Model {
  ...
  constructor() {
    ...
    this._position = {
      x: Model.CANVAS_WIDTH / 2 - 25,
      y: Model.CANVAS_HEIGHT / 2,
    };
    this._score = 0;
    this._level = 1;
  ...
  _getTargetPlatformCount() {
    ...
    const desired =
      maxPlatformsAtLevel1 - t * (maxPlatformsAtLevel1 - minPlatformsAtLevel10);
  ...
  _generatePlatforms() {
    ...
    while (this._platforms.length < targetCount) {
      ...
      this._platforms.push(
        new Platform(newPlatformX, newPlatformY, platformType)
      );
    }
  }
  ...
  _calculateViableXPosition(platformY) {
    ...
    if (viablePlatforms.length > 0) {
      ...
      const referencePlatform =
        viablePlatforms[Math.floor(Math.random() * viablePlatforms.length)];
